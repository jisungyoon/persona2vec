...
class Node2Vec(object):
        ...
        while len(walk) < walk_length:
            ...
            if len(cur_nbrs) > 0:
                if len(walk) == 1:
                    ...
                    next = cur_nbrs[
                        alias_draw(
                            alias_edges[(prev, cur)
                                        ][0], alias_edges[(prev, cur)][1]
                        )
                    ]
                    walk.append(next)
            else:
    ...
    def initialize_persona_vectors(self, base_emb, persona_to_node):
        ...
        self.model.build_vocab_from_freq(
            walk_counter, corpus_count=len(self.walks))
    ...
    def learn_embedding_one_epoch(self):
        self.model.train(
            self.walks, total_examples=self.model.corpus_count, epochs=1)
        self.embedding = {
            node: self.model.wv[str(node)] for node in self.G.nodes()}
