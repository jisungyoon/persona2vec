...
class TestPersona2VecGCN(unittest.TestCase):
    ...
    def test_splitting_algorithm(self):
        dimensions = 64
        X = pvgcn.laplacian_eigenmap(self.G, dimensions = 16)
        model = Persona2VecGCN(self.G, X = X, lambd=0.1, dimensions = dimensions, clustering_method="modularity")
        model = Persona2VecGCN(self.G, X = X, lambd=0.1, dimensions = dimensions, clustering_method="label_prop")
